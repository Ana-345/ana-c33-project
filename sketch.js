...
function keyPressed() {
  ...
  if (keyCode === 32) {
    ...
    var arrow = new computerArrow(posX, posY, 100, 10, angle);
...
function keyReleased() {
  if (keyCode === 32) {
    if (playerArrows.length) {
      ...
      if (keyCode === 32) {
        if (computerArrows.length) {
          var angle = computererArcher.body.angle;
...
function handlePlayerArcher() {
  if (!playerArcher.collapse && !computerArcher.collapse) {
    setTimeout(() => {
      ...
      if (move === "UP" && playerArcher.body.angle < 1.87) {
        angleValue = 0.1;
      }else{
          angleValue = -0.1;
      }
